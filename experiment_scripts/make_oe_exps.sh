...
exp_name=oe_model-${m}_m-5_r-96_intns-${intensity}_a-${alpha}_b-${beta}_g-${g}_d-${d}_p-${p}_aux-cor-${corr}
echo "#!/bin/bash
...
Rscript $WORKDIR/optimal_design_site_occ.R --working_dir=$WORKDIR --exp_name=$exp_name --data_reps=$(($cores*2)) --m=5 --model_selection=$m --random_starts=3 --exch_iter=20 --mcmc_iter=1000 --intensity_fun=\"$intensity\" --v_parallel --cores=$cores --alpha=$alpha --beta=$beta --gamma=$g --delta=$d --p=$p" --aux_cor=$corr > $exp_name.sh
