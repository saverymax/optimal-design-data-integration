...
for b in $beta
do
Rscript $WORKDIR/generate_po_data.R --working_dir=$WORKDIR --exp_name=po_gen_peak --alpha=$alpha --beta=$b --gamma=$g --delta=$d --intensity_func="donut" --bias_fun="exponential" --data_reps=$data_reps
done
done
done
