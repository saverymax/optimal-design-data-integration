...
then
	echo "no vary"
	exp_name=oe_model-${m}_m-5_n-${n}_v-none_a-${alpha}_b-${beta}_g-${g}_d-${d}_p-${p}
else
	echo "vary"
	exp_name=oe_model-${m}_m-5_n-${n}_v-vary_a-${alpha}_b-${beta}_g-${g}_d-${d}_p-${p}
fi
echo "#!/bin/bash
...
Rscript $WORKDIR/optimal_design_site_occ.R --working_dir=$WORKDIR --exp_name=$exp_name --data_reps=$(($cores*2)) --m=5 --min_visits=1 --max_visits=$n ${v}--model_selection=$m --random_starts=10 --exch_iter=40 --mcmc_iter=1000 --use_sim_po --po_data_file="po_gen_ints-${intensity}_a=${alpha}_b=${beta}_g=${g}_d=${d}.Rds" --intensity_func=\"$intensity\" --bias_func=\"$bias\" --v_parallel --cores=$cores --alpha=$alpha --beta=$beta --gamma=$g --delta=$d --p=$p" > $exp_dir/$exp_name.sh
