...
m=10
detection_p="0.2"
po_sample_prop="0.01 0.05 0.1 0.2 0.5 1"
n_surveys="5"
...
do
exp_name=oe_application_model-${model}_m-5_n-${n_surveys}_r-$(($cores*2))_p-${detection_p}_po_prop-${po_p}
echo "#!/bin/bash
...
Rscript $WORKDIR/optimal_design_application.R --working_dir=$WORKDIR --save_dir=experimental_runs/$exp_dir --base_data_dir=$BASE_DATA_DIR --data_save_dir=$save_dir --ebird_data_dir=$ebird_dir --map_file=$map_file --landcover_file=$landcover_file --modis_file=$modis_file --elevation_file=$elev_file --exp_name=$exp_name --data_reps=$(($cores*2)) --m=$m --min_visits=1 --max_visits=$n_surveys --random_starts=10 --model_selection=$model --p=$detection_p --po_sample_prop=$po_p --exch_iter=40 --mcmc_iter=1000 --v_parallel --cores=$cores" > $exp_dir/$exp_name.sh
