...
for n in $n_surveys
do
exp_name=oe_model-${m}_sample_effort-n-${n}_peak-${dev}_a-${alpha}_b-${b}_g-${g}_d-${d}_p-${p}
echo "#!/bin/bash
...
Rscript $WORKDIR/optimal_design_site_occ.R --working_dir=$WORKDIR --save_dir=experimental_runs/$exp_dir --exp_name=$exp_name --data_reps=$(($cores*2)) --m=5 --min_visits=1 --max_visits=$n --vary_visits --model_selection=$m --random_starts=10 --exch_iter=40 --mcmc_iter=1000 --use_sim_po --po_data_file="po_gen_peak_ints-${intensity}_peak=${dev}_a=${alpha}_b=${b}_g=${g}_d=${d}.Rds" --intensity_func=\"$intensity\" --sd=${dev} --bias_func=\"$bias\" --v_parallel --cores=$cores --alpha=$alpha --beta=$b --gamma=$g --delta=$d --p=$p" > $exp_dir/$exp_name.sh
