...
print(p)
```
Then specify the model. See work in "site_occupancy_stan.qmd" for the model equations and discussion.
...
```
There are a few assumptions we make in this work. First, in the Reich 2018 paper, due to symmetry of the sampling surface, they optimize over only a quarter of the grid, exploring $n/4$ sites. Secondly, here as in there, the sites are assumed to either be visited no times, or $n_i$ times, though this could easily be made variable for each site. It may be of interest to include variable numbers of visits to the site, as sites that have low detection probability may need more visits.

The exchange algorithm proceeds as follows: 

1. We first select $m$ sites for which $Y_i = n_i$, the number of surveys at this particular site.
2. We compute the criteria $V(Z)$ for the initial site Configuration.
3. Next, iterating through each site, we select a nearest neighbor for site $s_i$ and compute $V(Z)$. We see if this lowers $V(Z)$.
3. We update the site coordinates whenever a neighbor lowers $V$. While we could iterate through all neighbors of a site, per site, we instead iterate by only selecting 1 random neighbor at a time for 1 site, and then move to the next site. 
4. After one complete iteration through the sites, we check to see if the site locations are different than before we started the iteration through neighbors. If no, we end the loop.
```{r coordinate-ex}
...
We compare the minimum $V(Z)$ for different models, including those with and without prior information. The idea is to see if using the presence-only data allows us to better minimize the design criteria.
