...
print(n_f)
exp_col <- c("model-1_b-0.5", "model-1_b-1", "model-1_b-2", "model-3_b-0.5", "model-3_b-1", "model-3_b-2", "model-4_b-0.5", "model-4_b-1", "model-4_b-2", "model-5_b-0.5", "model-5_b-1", "model-5_b-2")
...
names(visit_counts) <- exp_col
names(var_list) <- exp_col
...
for (i in 1:length(file_list)){
  ...
  print(params)
  compare_name <- paste(params[[1]][2], params[[1]][7], sep="_")
  print("Name to identify run")
  print(compare_name)
  param_perm <- paste(params[[1]][3], params[[1]][9], sep="_")
  ...
  if (substr(file_name, str_length(file_name)-3, str_length(file_name))!= "xlsx"){
    ...
    print(visit_results)
    # random starts is fixed at 10
    n_visits <- visit_results[1:nrow(visit_results), 11:20]
    print("subset of visit xlsx")
    print(n_visits)
