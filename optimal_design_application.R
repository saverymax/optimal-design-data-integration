...
parser <- add_option(parser, "--model_selection", type="integer", default=1, help="Occupancy model to use: (1) with or (2) without PO data. Further details are provided in the modelling section of the documentation.")
parser <- add_option(parser, "--data_reps", type="integer", default=4, help="Number of dataset reps for criterion estimation. The V criterion is estimated as an expectation through numerical integration. data_reps is the number of integration points.")
parser <- add_option(parser, "--po_sample_prop", type="double", default=0.1, help="Proportion of PO sites to sample. 1 will use all PO")
parser <- add_option(parser, "--random_starts", type="integer", default=3, help="Number of random starts to run the exchange. The exchange algorithm is run multiple times to average over the random m sites initially selected.")
