...
#ggsave(fig_name, plot=p, dpi=300, width=15, height=8, units="cm", bg="white", device="png", type="cairo")
```
Let's also add elevation in the mix.

There are many sources of elevation. We can use the appears app: https://appeears.earthdatacloud.nasa.gov/task/area, provide a polygon, and download the aster gdem elevation product. Aster is described here: https://lpdaac.usgs.gov/products/astgtmv003/. Some more FAQ: https://www.usgs.gov/faqs/where-can-i-get-global-elevation-data

Or we can select the elevation download from https://www.earthenv.org/topography. The difference is the resolution, where the Aster product is 30m and the earthenv ranges from 1km to larger.
```{r elevation}
...
# Take another look at unique classes in the landcover data for Tennessee
unique(crop_lc_rast)
```
...
Based on this description, we select a subset of the landcover classes to use within the occupancy models. Additionally, note that not all of the landcover classes occur within Tennessee, and these are not included in the model, e.g., "Closed forest, deciduous 
needle leaf" (label 113) and "Open forest, mixed" (label 125). 

For occupancy probability we use the following variables (with classification ids):
...
We intentionally do not use closed forest deciduous broad leaf, and open forest unknown (unknown is very noisy). Open forest mixed does not occur in the landcover in 
the region of interest. However, closed forest unknown does correlate with the BH nuthatch range, even though the habitat is stated to not necessary include closed forests.

For detection probability we use
...
Closed forests are more difficult for volunteers to survey.
...
class(occ)
covar_names <- c(colnames(occ)[25:length(colnames(occ))], "latitude", "longitude")
covar_names
length(covar_names)
...
covar_labels <- c("shrubs", "herbaceous_vegetation", "agriculture", "urban", "bare/sparse", "water", "wetland", "closed_forest_evergreen_needle", 
                  ...
                  "open_forest_decid_broad leaf", "open_forest_unknown", "evi", "elevation", "latitude","longitude")
covar_labels
names(covar_names) <- covar_labels
covar_names

# Select covars to use for occupancy here
select_covar <- covar_names[c(2, 3, 8, 10, 11, 12, 13, 15, 16, 17, 18)]
select_covar
names(select_covar)
as.character(select_covar)

# Take a look at all the covariates by plotting rasterized versions of them, which is faster
for(i in 1:length(covar_names[1:(length(covar_names)-4)])){
  covar <- covar_names[i]
...
occ_model <- occu(~ duration_minutes + frac_50 + frac_111 + frac_114 + frac_115 + frac_116 +
                    number_observers + effort_distance_km + time_observations_started + protocol_type
                  ~ frac_30 + frac_40 + frac_111 + frac_115 + frac_116 + frac_121 + frac_124 + evi + elev,
                  data = occ_um)
simple_occ_model <- occu(~ duration_minutes + frac_50 + frac_116 +
                    number_observers + effort_distance_km + time_observations_started + protocol_type
                  ~ frac_111 + frac_115 + frac_116 + frac_121 + frac_124 + evi + elev,
                  data = occ_um)
basic_occ_model <- occu(~ duration_minutes + frac_50 + frac_116 +
                    effort_distance_km + time_observations_started 
                  ~ frac_121 + frac_124 + evi,
                  data = occ_um)
summary(occ_model)
summary(simple_occ_model)
summary(basic_occ_model)
...
# Rasterize based on the geometry of the buffers, as these are my "sites"
