...
k_params <- length(colnames(covar_df)) 
dim(param_vec)
...
head(covar_df)
sum(covar_df$counts)
# Okay, so do I fit PO model and draw from posterior or fit the SO model or what.

generate_ebird_pa <- function(data_reps, surface_data, p_0, k, n, sites, link){
  ...
  for (r in 1:data_reps){
    ...
    param_vec <- as.matrix(rnorm(k, 1, 0.5))
    if (link=="cloglog"){
...
# Oops somethings not right here.
pa_gen <- generate_ebird_pa(data_reps, covar_df, p, k, n, sites, link)
pa_gen$occupancy[1,2]
pa_gen$Y[1,2]
sum(pa_gen$Y)
pa_gen$theta[1,5]
data_gen_df <- as.data.frame(pa_gen$occupancy[1,])
data_gen_df
dim(data_gen_df)
data_gen_df$geometry <- subgrid_covars$geometry
subgrid_covars$geometry
occ_gen_sf <- st_as_sf(data_gen_df, coords="geometry")
...
```{r stan-model}
# TODO: Get stan model prepped for block matrix form for the parmaeters, which is useful to know how to do anyway.
# TODO: Generate PA data. 
poisson_process_site_occupancy <- '
  data{
    ...
    parameters{
      ...
      real gamma;
      vector[K] beta;       // vector of params for intensity
      vector[K] delta;      // vector of params for bias
    ...
    model{
      ...
      beta ~ multi_normal(0, 10)
      delta ~ multi_normal(0, 10)
