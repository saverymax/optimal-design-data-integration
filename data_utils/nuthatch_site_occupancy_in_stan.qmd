...
This report contains code to process ebird data and covariates, and to fit a stan site occupancy model using the nuthatch data as a prior. See the markdown "ebird_nuthatch_occupancy.rmd" for additional information regarding processing and the initial work I did getting the data ready for modelling.
...
```
We first load the state polygon and the preprocessed Ebird data. "ebird_nuthatch_occupancy.qmd" discusses the steps associated with processign this data. The function "load_ebird" will load this preprocessed data. In the process_ebird.R script, we can generate the preprocessed ebird data via "initial_auk_processing(base_data_dir, ebd_download_dir)"

We also load the landcover, EVI, and elevation raster data. These will be processed based on a grid buffer below.

```{r load-initial-data}
map_path <- file.path("us_states/GOVTUNIT_Tennessee_State_GPKG/GOVTUNIT_Tennessee_State_GPKG.gpkg")
...
# Yay we can use either version of terra or sf transform
...
stopifnot(st_crs(state_po_prj)==st_crs(prj_state))
p <- ggplot() + 
...
We then make the grid-based buffer, in order to extract the raster values within each grid cell. This is done by specifying a cell size (10000mx10000m). Once the buffer is created, we aggregate the point PO observations by cell, creating a Poisson count variable.

Using the exact_extract function, we can take aggregate the raster values as well. For elevation and EVI, we can just take the mean. But for the landcover, which is a categorical raster, things are a little more complicated. As per the recommendations at https://ebird.github.io/ebird-best-practices/, we convert the the categorical rasters to proportions within each buffered grid cell. This is done in the function "generate_gridded_covariates".

Once we have done the extraction, there is the matter of selecting which covariates to use in the site occupancy model, for both the intensity and the bias. From the landcover, we have the following categories

    covar_labels <- c("shrubs", "herbaceous_vegetation", "agriculture", "urban", "bare/sparse", "water", "wetland", "closed_forest_evergreen_needle",  "close_forest_decid_broad", "closed_forest_mixed", "closed_forest_unknown", "open_forest_evergreen_needle",  "open_forest_decid_broad leaf", "open_forest_unknown")
    
The manual checking and selection will be done in the next code block.
```{r process-covar}
...
# We are also going to experiment with downsampling the PO data.
#sample(state_po_prj, po_sample_n, replace=F)

# Then create counts
n_po_sample <- round(.01 * nrow(state_po_prj))
n_po_sample
po_id <- sample(1:nrow(state_po_prj), n_po_sample, replace=F)
state_po_prj_sample <- state_po_prj[po_id, ]
# Compute the counts using the sample
...
print(p)
```
Based on some preliminary modelling, we narrow down the covariates to use to model the intensity to
    intensity_covars <- covar_df[c("evi", "elev", "frac_111", "frac_115", "frac_124")]. These were relatively important to the model, and align with the observed PO pattern. This is however a rough means to select variables, but since we are not trying to perfectly specify a model and use one that has just enough predictive power to be useful, this variable selection is adequate for such purposes.
...
we now proceed to modelling. First we have to generate Presence-Absence data. To do so, we are going to nee
...
to the data and use these estimates to generate the PA data. Or we can fit the Presence-only model, and sample from the posterior of this, for each repetition of the PA data. Or, instead of sampling from the posterior, we can take the expectation of the parmeters.

```{r fit-ppd}
model_path <- "nuthatch_poisson_process.stan"
model_string <- nuthatch_poisson_process
write(model_string, model_path)
data_site_occ = list(N=sites, X=intensity_covars, y=covar_df$counts, Z=bias_covars, k_i=k_param_intn, k_b=k_param_bias, area_a=area_a)
...
print(p)
```
Once the PP is fit, we take the expectation for the parameters. We use these expectations to generate the PA data, such that we repetitively draw from the binomial to generate PA counts. The "generate_ebird_pa" function takes care of this. Then, we take a look at the generated datasets. We hope the these PPD values are reasonable within the expected Brown-headed Nuthatch pattern.
```{r PA-data}
...
TODO: Check out some other resources about occupancy modelling in stan: UBMS, Richard erickson "occupancy modelling in stan"

Using the PO data and generated PA data, we can now fit our site-occupancy model. We randomly sample $m$ sites for the PA likelihood, as is the procedure in the optimal design algorithm.
```{r model-fit}
...
selected_counts
site_visits <- rep(n_surveys, m)
data_site_occ = list(n_surveys=site_visits, n_pa_sites=m, n_po_sites=sites, X=covars_sampled_sites, Y=selected_counts, PO=covar_df$counts,
                         X_po=intensity_covars, Z_po=bias_covars, k_i=k_param_intn, k_b=k_param_bias, model_diag=0, area_a=area_a)
...
fit <- model$sample(data=data_site_occ, seed=13, chains=3, iter_sampling=2000, iter_warmup=500) 
```
We are having some issues fitting this model to the PA/PO data. These issues are in the process of being evaluated.
```{r model-inspect}
#fit$summary()
posterior <- fit$draws(all_params)
...
print(p)
```
That summarizes the work needed to generate PA data using the prior predictive distribution from a PP when we have real data. We are therefore prepared to use this procedure to generate data for the evaluation of optimal designs.
