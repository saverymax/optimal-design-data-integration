...
parser <- add_option(parser, "--exp_name", type="character", default="oe_run", help="Name of current experiment, which is used to name unique dir to save output")
parser <- add_option(parser, "--use_sim_po", action="store_true", default=F, help="Use PO generated by simulation. Check on user so that PO data is pre-simulated beforehand.")
...
parser <- add_option(parser, "--exch_iter", type="integer", default=3, help="Number of iterations of exchange before ending optimization. Recommended is 20 but default is set low for test runs.")
parser <- add_option(parser, "--mcmc_iter", type="integer", default=1000, help="Number of MCMC iterations in Stan")
parser <- add_option(parser, "--intensity_func", type="character", default="donut", help="Intensity function for sampling surface. This can be set as 'donut' or 'simple'")
parser <- add_option(parser, "--bias_func", type="character", default="exponential", help="Bias function for sampling surface. Currently can be generated as either an 'exponential' mountain from a single point in the region, or with 'correlation' as a donut correlated with the intensity.")
parser <- add_option(parser, "--p_logging", action="store_true", default=F, help="Boolean for logging information about posterior estimates fit in Stan, when using nonparallel code. The implementation of this is currently not tested and should be left as False")
parser <- add_option(parser, "--debug_stan", action="store_true", default=F, help="Boolean for turning on messages from stan model in nonparallel V. Useful to see any issues during fitting.")
parser <- add_option(parser, "--v_parallel", action="store_true", default=F, help="Boolean for parallel computation of V criterion, if you have have multiple cores available that are compatible with the R parallel library")
...
parser <- add_option(parser, "--beta", type="double", default=0.5, help="Slope for intensity. The parameters for that specify the intensity and models will be explained in greater detail in the modelling section.")
...
parser <- add_option(parser, "--sd", type="double", default=5, help="Standard deviation for donut intensity surface. Not permuted during experiments but it can have a large effect on exchange algorithm.")
...
print(exp_args)
# p_logging not tested at the moment
